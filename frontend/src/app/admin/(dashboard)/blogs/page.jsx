...
export default function BlogsPage() {
  ...
  const refresh = useCallback((q = "") => {
    ...
    adminApi.listBlogs(query).then((res) => {
      setBlogs(res.data);
      setLoading(false);
    });
  }, []);
    ...
    <div className="min-h-screen bg-[#F5EBE0]">
      <header className="flex items-end justify-between px-8 sm:px-11 pt-9 pb-6 border-b border-[#3D1F0D]/10">
        <div>
          ...
          <h1 className="font-display text-3xl text-[#3D1F0D]">Blog Posts</h1>
        </div>
        <button
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${
                              b.isPublished ? "bg-emerald-500" : "bg-[#3D1F0D]/30"
                            }`}
                          />
            ...
            <div className="text-center py-16 px-6">
              <p className="text-sm text-[#3D1F0D]/40 mb-4">
                {search
                  ...
                  : `No blog posts yet — click "New" to write your first one.`}
              </p>
              {!search && (
                <button
                  onClick={() => setDrawer({ blog: null })}
