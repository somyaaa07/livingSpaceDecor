...
export default function ProjectsPage() {
  ...
  const refresh = useCallback((q = "") => {
    ...
    return adminApi.listProjects(query).then((res) => {
  ...
  useEffect(() => {
    refresh().then((data) => {
      ...
      if (editId) {
        const match = data.find((p) => String(p.id) === editId);
        if (match) setDrawer({ project: match });
  ...
  const RowActions = ({ project }) => (
    <div className="flex flex-nowrap gap-2 lg:justify-end">
      <Link
    ...
    <div className="min-h-screen bg-[#F5EBE0]">
      <header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 px-5 sm:px-8 lg:px-11 pt-7 sm:pt-9 pb-5 sm:pb-6 border-b border-[#3D1F0D]/10">
        <div>
          ...
          <h1 className="font-display text-2xl sm:text-3xl text-[#3D1F0D]">Projects</h1>
        </div>
        <button
                ...
                >
                  <div className="flex gap-3 mb-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-sm text-[#3D1F0D] truncate">{p.name}</p>
          ...
          {!loading && projects.length === 0 && (
            <div className="text-center py-16 px-6 bg-white border border-[#3D1F0D]/10 rounded-xl">
              <p className="text-sm text-[#3D1F0D]/40 mb-4">
                {search ? `No projects match "${search}".` : `No projects yet — click "New" to add your first one.`}
              </p>
              {!search && (
                <button
                  onClick={() => setDrawer({ project: null })}
