...
export const metadata = { title: "Portfolio — Studio" };

export default async function PortfolioPage({ searchParams }) {
  const { page: pageParam } = await searchParams;
  const page = parseInt(pageParam, 10) || 1;
  const { data: projects, pagination } = await getProjects({ page, limit: 12 });
  ...
  return (
    <main className="min-h-screen bg-[#F5EBE0] text-[#3D1F0D]">
      ...
      <section className="max-w-6xl mx-auto px-6 md:px-16 pt-16 pb-8 text-center">
        ...
        <p className="text-[#3D1F0D]/60 max-w-xl mx-auto font-light">
          A collection of interiors we've designed and built, room by room.
        </p>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-16 pb-24">
        {projects.length === 0 ? (
          <p className="text-center text-[#3D1F0D]/50 py-20">No projects published yet.</p>
        ) : (
        ...
        <GridPagination basePath="/portfolio" pagination={pagination} />
      </section>
    </main>
  );
}